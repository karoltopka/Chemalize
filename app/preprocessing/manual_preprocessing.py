...
def handle_missing_values(df, method='drop_rows', constant_value=0, na_values=None):
    """
    ...
    Args:
        ...
        na_values (list): Lista wartości do zamiany na NaN przed przetwarzaniem
    ...
    # NOWE: Domyślne wartości do konwersji na NaN
    if na_values is None:
        na_values = ['na', 'NA', 'n/a', '', 'null', 'NULL', 'None', 'nan', 'NaN']
    
    # NOWE: Konwertuj stringi na NaN PRZED rozpoczęciem przetwarzania
    if na_values:
        result_df = result_df.replace(na_values, np.nan)
...
def process_dataset(df, action_type, params=None):
    ...
    if action_type == 'scale':
        ...
        # NOWE: Przekaż na_values do handle_missing_values
        na_values = params.get('na_values', None)  # None = użyj domyślnych wartości
        
        result_df, stats = handle_missing_values(result_df, method, constant_value, na_values)
