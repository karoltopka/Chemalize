...
def get_clean_path():
    ...
    return config_get_clean_path(csv_filename)
...
def xy_plot(feature_x, feature_y, df=None):
    ...
    Args:
        feature_x: Name of the X-axis column
        feature_y: Name of the Y-axis column
        df: DataFrame to plot (optional, if not provided will read from col.csv)
    """
    try:
        print(f"Starting xy_plot with {feature_x} and {feature_y}")

        # If DataFrame is provided, use it directly (preferred method)
        if df is not None:
            print(f"Using provided DataFrame, shape: {df.shape}")
        else:
            # Fallback: Load from col.csv using absolute path
            # __file__ is in app/visualization/visualize.py, so go up 2 levels to get to Chemalize root
            base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            col_csv_path = os.path.join(base_dir, "data", "temp", "col.csv")
            print(f"Looking for col.csv at: {col_csv_path}")
            df = pd.read_csv(col_csv_path)
...
def static_hist_plot(df, feature_x):
    try:
        ...
        # Używamy scentralizowanego katalogu app/static/img
        os.makedirs(STATIC_IMG_DIR, exist_ok=True)

        # Ścieżka do zapisania pliku
        file_name = f"hist_{feature_x}.png"
        abs_plot_path = os.path.join(STATIC_IMG_DIR, file_name)
